...
<template>
  <div class="relative inline-block">
    <!-- <UButton 
      ...
      variant="soft"
      @click="isOpen = !isOpen"
      class="language-button border-primary-700 hidden md:inline-flex"
    >
      <template #leading>
    ...
    <UTooltip :delay-duration="0" :text="$t('common.languages') || 'Languages'" :shortcuts="[]" :popper="{ placement: 'bottom' }">
      <Button
        variant="tertiary"
        :class="[
          'border-primary-700 relative',
          isOpen && 'bg-primary-200 shadow-md'
        ]"
        @click="isOpen = !isOpen"
    ...
    >
      <div
        v-if="isOpen"
        class="absolute top-full mt-1 w-max bg-neutral-100 border border-primary-700 rounded-md z-50 overflow-hidden"
      >
        <button
          v-for="lang in languages"
          :key="lang.code"
          @click="selectLanguage(lang.code)"
          class="w-full px-3 py-2.5 flex items-center gap-2.5 hover:bg-neutral-150 border-0 transition-colors text-left text-sm font-medium"
